...
$guardPath = Join-Path $repoRoot "stack\open-webui-functions\kahle_toolcall_guard.py"
...
docker exec $Container sh -c "mkdir -p /tmp/kahle-vinci/scripts/openwebui /tmp/kahle-vinci/stack/open-webui-tools/dist /tmp/kahle-vinci/stack/open-webui-functions"
...
docker cp $guardPath "${Container}:/tmp/kahle-vinci/stack/open-webui-functions/kahle_toolcall_guard.py"
if ($LASTEXITCODE -ne 0) { throw "Toolcall-Guard konnte nicht kopiert werden." }
...
foreach ($tool in @("rag_chat", "kahle_workflow", "kahle_toolcall_guard")) {
    docker exec `
...
Write-Host "RAG-Tools und Toolcall-Guard wurden ohne API-Key aktualisiert. OpenWebUI startet neu."
