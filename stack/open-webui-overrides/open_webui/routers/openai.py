...
_MODEL_LIST_RETRIES = max(1, int(os.getenv('IONOS_MODEL_LIST_RETRIES', '3')))
_MODEL_LIST_RETRY_DELAY_SECONDS = max(0.0, float(os.getenv('IONOS_MODEL_LIST_RETRY_DELAY_SECONDS', '0.5')))
_MODEL_FALLBACK_IDS = tuple(
    model_id.strip()
    for model_id in os.getenv(
        'IONOS_MODEL_FALLBACK_IDS',
        'mistralai/Mistral-Small-24B-Instruct,openai/gpt-oss-120b,Qwen/Qwen3.5-397B-A17B',
    ).split(',')
    if model_id.strip()
)
...
async def send_get_request(
    ...
    last_error = None
    for attempt in range(_MODEL_LIST_RETRIES):
        try:
            ...
            last_error = exc
            if attempt + 1 < _MODEL_LIST_RETRIES:
                await asyncio.sleep(_MODEL_LIST_RETRY_DELAY_SECONDS * (attempt + 1))

    log.error(
        'Model list connection failed after %s attempts: %s',
        _MODEL_LIST_RETRIES,
        last_error,
    )
    return None
...
def _model_list_request_failed(responses) -> bool:
    return any(
        response is None or (isinstance(response, dict) and 'error' in response)
        for response in (responses or [])
    )


def _configured_model_fallback() -> dict[str, dict]:
    return {
        model_id: {
            'id': model_id,
            'name': model_id,
            ...
            'provider': 'ionos-fallback',
            'urlIdx': 0,
        }
        for model_id in _MODEL_FALLBACK_IDS
    }


def _recover_model_catalog(
    models: dict[str, dict],
    previous_models: dict[str, dict],
    responses,
) -> tuple[dict[str, dict], str]:
    if models or not _model_list_request_failed(responses):
        return models, ''
    if previous_models:
        return previous_models, 'last-known-good'
    return _configured_model_fallback(), 'configured-fallback'
...
async def get_all_models(request: Request, user: UserModel) -> dict[str, list]:
    ...
    previous_models = dict(getattr(request.app.state, 'OPENAI_MODELS', {}) or {})
    ...
    models, recovery_source = _recover_model_catalog(models, previous_models, responses)
    if recovery_source:
        log.warning('Using %s model catalog after provider model-list failure', recovery_source)
