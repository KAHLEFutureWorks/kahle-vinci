...
def get_output_text(output: list | None) -> str:
    """Extract user-visible text from OpenAI Responses API output items.

    Open WebUI 0.11 imports this helper while loading its database models.
    Keep this compatibility helper in the KAHLE override so the custom host
    filtering and metadata sanitisation remain active.
    """
...
def reconcile_tool_pairs(messages: list[dict]) -> list[dict]:
    ...
    Stored output can be incomplete â€” a tool result may be missing (e.g. the
    ...
    for message in messages:
        ...
        # Orphan tool result â€” no assistant ever claimed this call_id.
        if role == 'tool' and message.get('tool_call_id') not in requested_tool_call_ids:
            continue

        # Non-assistant or no tool_calls â€” pass through unchanged.
        ...
        # All tool_calls were orphans â€” keep the message only if it
...
def get_last_user_message(messages: list[dict]) -> Optional[str]:
    ...
    include_file_names = (
        os.getenv("OWUI_APPEND_ATTACHED_FILE_NAMES_TO_USER_MESSAGE", "true")
        .strip()
        .lower()
        in ("1", "true", "yes", "on")
    )
    ...
    file_names = _extract_file_names_from_message(message)
    if not file_names:
        return content or None
...
def _strip_null_bytes_deep(obj):
    """Inner recursive walk â€” only called when null bytes are known to be present."""
