...
async def _evict_stale_local_tool_cache(request: Request, tool_ids: list[str] | None) -> None:
    ...
    for tool_id in tool_ids:
        ...
        if not isinstance(current_content, str):
            continue
        if cached_contents.get(tool_id) != current_content:
...
async def chat_completion_tools_handler(
    ...
    def _infer_fallback_tool_calls(result_obj: Any, user_text: Optional[str]) -> list[dict[str, Any]]:
        ...
        if (
            ...
            and _looks_like_previous_result_file_request(text)
        ):
            return [
                {
...
async def streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter:
        ...
        async def response_handler(response, events):
            ...
            def display_output(extra_items: list | None = None):
                items = full_output()
                if extra_items:
                    items = items + extra_items
                return _stream_safe_output(items)
                            ...
                            if data:
                                ...
                                # Check for Responses API events (type field starts with "response.")
                                    ...
                                    if reasoning_content:
                                        ...
                                        data = {'content': serialize_output(display_output())}
