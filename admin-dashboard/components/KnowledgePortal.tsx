...
type UploadResult = { case_id: string; status: string; requires_admin: boolean; exact_duplicate_document_id?: string; matches: Match[]; confidentiality:string; confidentiality_reason:string };
...
export default function KnowledgePortal() {
  ...
  const refresh = useCallback(async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Portal konnte nicht geladen werden"); }
  ...
  useEffect(() => {
    ...
    try {
      ...
      void api(`/portal/admin/users/${userId}/role`, {
        ...
        .catch(cause => setError(cause instanceof Error ? cause.message : "Rollenänderung fehlgeschlagen"));
...
function Upload({ session, kbs, done }: { session: Session; kbs: KB[]; done: () => Promise<void> }) {
  ...
  async function submit() { if (!file || !kb || !title.trim()) return setMessage("Bitte Datei, Titel und Wissensbereich auswählen."); setBusy(true); setMessage(""); try { const form = new FormData(); form.append("file", file); form.append("knowledgebase_id", kb); form.append("title", title.trim()); form.append("valid_workdays", String(days)); form.append("confidentiality", confidentiality); const response = await fetch(`${API}/portal/documents`, { method: "POST", credentials: "include", body: form }); const body = await response.json(); if (!response.ok) throw new Error(body.detail || "Upload fehlgeschlagen"); setResult(body); await done(); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Upload fehlgeschlagen"); } finally { setBusy(false); } }
  async function action(value: string) { if (!result) return; setBusy(true); try { await api(`/portal/cases/${result.case_id}/action`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: value }) }); setResult(null); setFile(null); setTitle(""); setMessage("Deine Auswahl wurde weitergeleitet."); await done(); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Aktion fehlgeschlagen"); } finally { setBusy(false); } }
  return <section className="wp-page narrow"><Title eyebrow="Neues Wissen" title="Dokument bereitstellen" text="Du legst nur die Datei ab. Das Portal prüft Sicherheit, Aufbereitung und ähnliche Inhalte." />{!result ? <div className="wp-form"><label className={`wp-drop ${file ? "selected" : ""}`} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); setFile(e.dataTransfer.files[0] || null); }}>{file ? <><CheckCircle2 /><strong>{file.name}</strong><span>{(file.size / 1048576).toFixed(1)} MB · Datei ändern</span></> : <><FileUp /><strong>Datei hier ablegen</strong><span>oder klicken und auswählen · maximal 50 MB</span></>}<input type="file" accept=".pdf,.docx,.xlsx,.pptx,.txt,.md,.csv" onChange={e => setFile(e.target.files?.[0] || null)} /></label><div className="wp-grid"><label>Titel<input value={title} onChange={e => setTitle(e.target.value)} placeholder="Worum geht es?" /></label><label>Ziel-Wissensbereich<select value={kb} onChange={e => setKb(e.target.value)}><option value="">Bitte wählen</option>{kbs.map(item => <option key={item.knowledgebase_id} value={item.knowledgebase_id}>{item.label}</option>)}</select></label><label>Gültigkeit<select value={days} onChange={e => setDays(Number(e.target.value))}>{[60,45,30,20,10].map(value => <option key={value}>{value}</option>)}</select></label><label>Mindesteinstufung<select value={confidentiality} onChange={e => setConfidentiality(e.target.value)}><option value="internal">Intern</option><option value="restricted">Bereichsbeschränkt</option><option value="confidential">Vertraulich</option></select></label></div><div className="wp-owner"><ShieldCheck /><div><strong>Owner: {session.email}</strong><span>Erinnerungen werden automatisch deinem Konto zugeordnet.</span></div></div>{message && <p className="wp-message">{message}</p>}<button className="wp-primary" disabled={busy || !file || !kb} onClick={() => void submit()}>{busy && <LoaderCircle className="spin" />} Sicher prüfen</button></div> : <div className="wp-result">{result.status === "duplicate_blocked" ? <XCircle className="red" /> : result.requires_admin ? <AlertTriangle className="amber" /> : <CheckCircle2 className="green" />}<h2>{result.status === "duplicate_blocked" ? "Dokument bereits vorhanden" : result.matches.length ? "Ähnliche Inhalte gefunden" : "Prüfung abgeschlossen"}</h2><p>{result.requires_admin ? "Dieser Fall wird nach deiner Auswahl direkt einem Admin vorgelegt." : "Wähle, wie es weitergehen soll."}</p><div className="wp-owner"><ShieldCheck/><div><strong>Automatische Einstufung: {result.confidentiality}</strong><span>{result.confidentiality_reason}</span></div></div>{result.matches.map(match => <article key={match.document_id}><div><strong>{match.title}</strong><span>{match.has_conflict ? "Möglicher Widerspruch" : match.version_candidate ? "Mögliche neue Version" : "Ähnlicher Inhalt"}</span></div><Badge status={match.level} /></article>)}<div className="wp-actions">{!result.exact_duplicate_document_id && <button onClick={() => void action("create")}>Als neues Dokument vorschlagen</button>}{result.matches.some(m => m.version_candidate) && <button onClick={() => void action("replace")}>Als neue Version vorschlagen</button>}{result.exact_duplicate_document_id && <button onClick={() => void action("publish_existing")}>Vorhandenes zusätzlich veröffentlichen</button>}<button className="secondary" onClick={() => void action("discard")}>Verwerfen</button></div></div>}</section>;
}
...
function UserAdmin({ users, session, done }: { users: PortalUser[]; session: Session; done: () => Promise<void> }) {
  ...
  useEffect(() => { void api<any>("/portal/admin/delegations").then(payload=>setDelegations(payload.delegations)); }, []);
  useEffect(() => {
    if (!selected) return;
    void api<{access: UserAccess[]}>(`/portal/admin/users/${selected}/knowledgebase-access`)
      .then(payload => setAccess(payload.access)).catch(cause => setMessage(cause instanceof Error ? cause.message : "Rechte konnten nicht geladen werden"));
  }, [selected]);
  ...
  return <section className="wp-page"><Title eyebrow="Administration" title="Benutzer und Rechte" text="Rollen, Führungskräfte sowie Lese- und Uploadrechte werden hier zentral verwaltet." />
    ...
    <div className="wp-users">{users.map(user => <article key={user.user_id} className={selected === user.user_id ? "selected" : ""} onClick={() => setSelected(user.user_id)}><div className="wp-avatar">{user.display_name.slice(0,2).toUpperCase()}</div><div><strong>{user.display_name}</strong><span>{user.email}</span></div><select value={user.role} onClick={event => event.stopPropagation()} disabled={session.role !== "portal_admin" && ["admin","portal_admin"].includes(user.role)} onChange={e => void beginRoleChange(user.user_id,e.target.value as Role)}><option value="employee">Mitarbeiter</option><option value="manager">Führungskraft</option><option value="admin">Admin</option>{session.role === "portal_admin" && <option value="portal_admin">Portal-Admin</option>}</select><Badge status={user.active ? "active" : "Deaktiviert"} /></article>)}</div>
    ...
    {current && <div className="wp-form"><h2>Zuordnung für {current.display_name}</h2><label>Führungskraft<select value={current.manager_user_id || ""} onChange={e => void setManager(e.target.value)}><option value="">Keine Führungskraft</option>{managers.filter(user => user.user_id !== current.user_id).map(user => <option key={user.user_id} value={user.user_id}>{user.display_name}</option>)}</select></label><h2>Wissensbereiche</h2><div className="wp-kbs">{access.map(item => <article key={item.knowledgebase_id}><div><strong>{item.label}</strong><span>{item.knowledgebase_id}</span></div><label><input type="checkbox" checked={Boolean(item.can_read)} onChange={e => void setRight(item,"can_read",e.target.checked)} /> Lesen</label><label><input type="checkbox" checked={Boolean(item.can_upload)} onChange={e => void setRight(item,"can_upload",e.target.checked)} /> Hochladen</label></article>)}</div></div>}
  </section>;
}
...
function QualityDashboardView({data}:{data:any}) {
  ...
  const cards=[['Aktive Dokumente',data.active_documents],['Ablauf in ≤ 15 Arbeitstagen',data.expiring_within_15_workdays],['Offene Wissensfehler',data.open_feedback],['Offene Systemincidents',data.open_incidents],['Ausstehende Mails',data.mail?.pending],['Fehlgeschlagene Mails',data.mail?.failed]];
...
function DocumentList({documents,changes,ownershipTasks,users,session,done}:{documents:PortalDocument[];changes:any[];ownershipTasks:OwnershipTask[];users:PortalUser[];session:Session;done:()=>Promise<void>}) {
  ...
  async function proposeOwner(id:string){await api(`/portal/ownership-tasks/${id}/proposal`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({proposed_owner_user_id:proposedOwner,reason:reason||'Fachliche Zust?ndigkeit neu zugeordnet'})});await done();}
