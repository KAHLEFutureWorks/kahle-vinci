...
function Upload({ session, kbs, done }: { session: Session; kbs: KB[]; done: () => Promise<void> }) {
  ...
  async function submit() {
    ...
    try {
      ...
      form.append("title", title.trim()); form.append("valid_workdays", String(days)); form.append("confidentiality", confidentiality); form.append("owner_user_id", ownerUserId);
      ...
      let current: UploadJob = created; setJob(current);
      while (!['completed','failed'].includes(current.status)) {
        await new Promise(resolve => window.setTimeout(resolve, 700));
        current = await api<UploadJob>(`/portal/upload-jobs/${created.job_id}`); setJob(current);
      }
    ...
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Upload fehlgeschlagen"); }
