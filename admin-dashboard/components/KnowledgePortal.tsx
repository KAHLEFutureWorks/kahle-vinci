...
const changeKindText: Record<string, string> = {
  create: "Neu anlegen", rename: "Umbenennen",
  archive: "Archivieren", delete: "Endgültig entfernen",
};
...
const errorText: Record<string, string> = {
  ...
  knowledgebase_must_be_archived_first: "Ein Wissensbereich muss zuerst archiviert werden, bevor er endgültig entfernt werden kann.",
...
function KnowledgebaseAdmin({ changes, users, session, done }: { changes: KBChange[]; users: PortalUser[]; session: Session; done: () => Promise<void> }) {
  ...
  const [notice,setNotice]=useState(""),[confirmedTarget,setConfirmedTarget]=useState("");
  ...
  return <section className="wp-page"><Title eyebrow="Administration" title="Knowledge Bases verwalten" text="Normale Admins bereiten Änderungen vor; Portal-Admins geben sie frei oder führen sie direkt aus." /><div className="wp-form"><div className="wp-grid"><label>Aktion<select value={kind} onChange={e=>setKind(e.target.value)}><option value="create">Neu anlegen</option><option value="rename">Umbenennen</option><option value="archive">Archivieren</option><option value="delete">Endgültig entfernen</option></select></label>{kind!=="create"&&<label>Knowledge Base<select value={target} onChange={e=>setTarget(e.target.value)}><option value="">Bitte wählen</option>{overview.filter(base=>kind==="delete"?base.status==="archived":base.status==="active").map(base=><option key={base.knowledgebase_id} value={base.knowledgebase_id}>{base.label}{base.status==="archived"?" (archiviert)":""}</option>)}</select>{kind==="delete"&&<small className="wp-hint">Nur archivierte Wissensbereiche können endgültig entfernt werden.</small>}</label>}{["create","rename"].includes(kind)&&<label>Name<input value={label} onChange={e=>setLabel(e.target.value)}/></label>}{kind==="create"&&<label>Kurzname<input value={slug} onChange={e=>setSlug(e.target.value)} placeholder="z. B. service"/></label>}</div><label>Zweck oder Begründung<textarea value={reason} onChange={e=>setReason(e.target.value)}/></label>{notice&&<p className="wp-message">{notice}</p>}<button className="wp-primary" onClick={()=>void requestChange()}>{notice&&confirmedTarget===target?"Trotzdem einreichen":"Änderung einreichen"}</button></div><h2>Wissensbereiche und ihre Dokumente</h2><div className="wp-kb-overview">{overview.length ? overview.map(base=><article key={base.knowledgebase_id} className={open===base.knowledgebase_id?"selected":""}><button type="button" className="wp-kb-head" aria-expanded={open===base.knowledgebase_id} onClick={()=>setOpen(current=>current===base.knowledgebase_id?"":base.knowledgebase_id)}><ChevronRight className={open===base.knowledgebase_id?"wp-rot":""}/><span><strong>{base.label}</strong><small>{base.purpose||base.slug}</small></span><span className="wp-kb-count">{base.document_count} {base.document_count===1?"Dokument":"Dokumente"}</span></button>{open===base.knowledgebase_id&&<div className="wp-kb-docs">{base.documents.length ? <table><thead><tr><th>Titel</th><th>Status</th><th>Owner</th><th>Gültig bis</th></tr></thead><tbody>{base.documents.map(doc=><tr key={doc.document_id}><td>{doc.title}</td><td>{statusText[doc.status]||doc.status}</td><td>{doc.owner_name}</td><td>{doc.valid_until||"offen"}</td></tr>)}</tbody></table> : <p className="wp-empty-hint">Diesem Wissensbereich ist noch kein Dokument zugeordnet.</p>}</div>}</article>) : <p className="wp-empty-hint">Es ist noch kein Wissensbereich angelegt.</p>}</div><h2>Änderungsanträge</h2><div className="wp-doc-list">{changes.length ? changes.map(change=><article key={change.request_id}><div className="wp-trash-row"><div className="wp-trash-copy"><Badge status={change.status}/><strong>{changeKindText[change.kind]||change.kind}: {change.payload?.label||overview.find(base=>base.knowledgebase_id===change.knowledgebase_id)?.label||change.knowledgebase_id}</strong><span>Beantragt von {userNames[change.requested_by]||change.requested_by}</span><small className="wp-hint">{change.request_id}</small></div><div className="wp-trash-side">{session.role==="portal_admin"&&change.status==="pending"?<div className="wp-actions"><button onClick={()=>void decide(change.request_id,false)}>Ablehnen</button><button className="approve" onClick={()=>void decide(change.request_id,true)}>Freigeben</button></div>:<p className="wp-hint">{change.status==="pending"?"Wartet auf einen Portal-Admin.":"Bereits entschieden."}</p>}</div></div></article>) : <p className="wp-empty-hint">Kein offener Änderungsantrag.</p>}</div></section>;
}
...
function DocumentList({documents,changes,ownershipTasks,users,session,done}:{documents:PortalDocument[];changes:any[];ownershipTasks:OwnershipTask[];users:PortalUser[];session:Session;done:()=>Promise<void>}) {
  ...
  const [message,setMessage]=useState(""),[bases,setBases]=useState<KBOverview[]>([]),[targetBase,setTargetBase]=useState("");
  const isAdmin = ["admin","portal_admin"].includes(session.role);
  useEffect(()=>{if(!isAdmin)return;void api<{knowledgebases:KBOverview[]}>("/portal/admin/knowledgebase-overview")
    .then(payload=>setBases(payload.knowledgebases.filter(base=>base.status==="active"))).catch(()=>setBases([]));},[isAdmin]);
  ...
  const publish = (documentId:string, active:boolean) => run(
    active?"Die Zuordnung":"Das Lösen der Zuordnung",
    () => api(`/portal/admin/documents/${documentId}/publications`, {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({knowledgebase_id:targetBase,active,reason})}));
    ...
    <div className="wp-doc-list">{documents.length ? documents.map(doc=>{
      ...
      return <article key={`${doc.document_id}-${doc.knowledgebase_id}`} className={open?"selected":""}>
        ...
        {open&&<div className="wp-doc-panel">
          ...
          {isAdmin&&<div className="wp-publish">
            <label>Wissensbereich zuordnen oder lösen<select value={targetBase} onChange={e=>setTargetBase(e.target.value)}>
              <option value="">Bitte wählen</option>
              {bases.map(base=><option key={base.knowledgebase_id} value={base.knowledgebase_id}>{base.label}</option>)}
            </select></label>
            <div className="wp-actions">
              <button disabled={!targetBase} onClick={()=>void publish(doc.document_id,true)}>Zuordnen</button>
              <button disabled={!targetBase} onClick={()=>void publish(doc.document_id,false)}>Zuordnung lösen</button>
            </div>
          </div>}
          {message&&<p className="wp-message">{message}</p>}
        </div>}
